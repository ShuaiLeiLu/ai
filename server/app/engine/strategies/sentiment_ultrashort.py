...
def _passes_sentiment_liquidity_filters(
    *,
    ...
    if cap <= 0 or cap < min_cap or cap > max_cap:
        return False
    ...
    if turnover_ratio > filters.get("max_turnover_ratio", 25):
        return False
    return _passes_low_position_filter(quote, config)
    ...
    *,
    ...
    for stock in limit_up_pool:
        ...
        if latest_first_seal_int and _time_to_int(stock.first_seal_time) > latest_first_seal_int:
            continue
        if not _passes_sentiment_liquidity_filters(
            symbol=stock.symbol,
            name=stock.name,
            quote=quote,
...
async def execute(session: AsyncSession, researcher: Researcher) -> int:
    ...
    if remaining_new_orders > 0 and exposure_budget > 1000:
    ...
    for target in buy_targets:
        ...
        if symbol in current_positions or symbol in today_buys or symbol in seen_targets:
            continue
